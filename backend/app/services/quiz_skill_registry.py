...
QUIZ_SKILLS: dict[str, QuizSkill] = {
    "concept_explain": QuizSkill(
        ...
        generation_prompt=(
            "先从正文中定位一个明确命名或明确定义的核心概念，并在题面中直接写出这个概念；"
            "不得让读者自己选择概念。围绕正文对它的具体定义、性质或用途，要求读者向刚接触本章的人解释："
            "它描述或解决什么、哪一条正文中的关键性质决定了它，并用一个与正文内容相符的例子、反例或后果帮助理解。"
            "不要逐字考定义，不要只替换章节标题生成通用题，也不要把多个无关概念塞进一题。"
        ),
        evaluation_prompt=(
    ...
    "theorem_understanding": QuizSkill(
        ...
        required_context=("chapter_body", "chapter_guide"),
        generation_prompt=(
            "先定位正文中明确陈述的一个具体定理、命题或关键结论，并在题面中直接写出它的名称、结论对象或足以唯一定位它的内容；"
            "不得让读者自己选择定理。请读者用自然语言说明："
            "它从哪些关键条件出发、保证了什么、为什么这个结论值得关心；可进一步问去掉某个关键条件后直觉上会出什么问题。"
            "不要只用章节标题或‘一个关键定理’作定位，也不要在问题里泄露完整答案。"
        ),
        evaluation_prompt=(
    ...
    "proof_strategy": QuizSkill(
        ...
        generation_prompt=(
            "先定位正文中确实给出证明或推导思路的一个具体结果，在题面中直接写出要证明的结果，以及正文采用的关键工具、构造或转折之一；"
            "不得让读者自己选择证明。请读者口头讲解这段证明的路线："
            "从哪里出发、最关键的构造/引理/观察是什么、它怎样把问题推进到结论。"
            "可以聚焦一个关键转折，但不要只写‘本章中的一个重要证明’，不要挖掉某行公式让用户补写，也不要要求完整形式化证明或计算。"
        ),
        evaluation_prompt=(
    ...
    "concept_connection": QuizSkill(
        ...
        generation_prompt=(
            "先定位正文中明确存在联系的两个具体概念或结果，并在题面中直接写出两者；不得让读者自己选择。请读者说明："
            "两者各自做什么、联系的方向是什么、为什么需要把它们放在一起理解。"
            "关系必须能由正文中的同一段论述、定义依赖或证明用途支持；不要只用章节标题作定位，"
            "不要凭名称相似硬凑比较，也不要只要求列相同点/不同点。"
        ),
        evaluation_prompt=(
