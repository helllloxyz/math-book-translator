...
assert.match(component, /class="dialog-toolbar"/, 'input footer should expose quick-input buttons')
assert.match(component, /v-if="!isQuiz" class="dialog-toolbar"/, 'note quick inputs should stay out of the teach-back quiz composer')
...
assert.match(component, /if \(!Array\.isArray\(rawInputs\)\) return DEFAULT_QUICK_INPUTS/, 'dialog should fall back to default quick inputs only for invalid config shapes')
assert.doesNotMatch(component, /quickInputs\.length \? quickInputs : DEFAULT_QUICK_INPUTS/, 'dialog should honor an intentionally empty quick-input config')
...
assert.match(component, /message\.suggestedQuestions/, 'follow-up links should come from the assistant response instead of fixed templates')
assert.doesNotMatch(component, /这段话的关键假设是什么|后续证明会用到哪一步/, 'follow-up links should not use fixed question templates')
assert.match(component, /v-html="renderInlineMarkdown\(question\)"/, 'follow-up links should render inline Markdown and LaTeX')
